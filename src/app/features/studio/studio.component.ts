...
export class StudioComponent {
  ...
  getProductImageUrlForAngle(angle: ViewAngle): string {
    ...
    const images = Array.isArray(selectedProduct?.images)
      ...
      : [];
    const activeImage = images.find(
      (image) => image.viewAngle === angle,
    );
    const imageUrl =
      activeImage?.imageUrl || (angle === ViewAngle.Front ? selectedProduct?.image : "") || this.logo;
  ...
  async saveToDrafts(): Promise<void> {
    ...
    try {
      ...
      const base64Front = await this.designCanvas.captureViewSnapshot('front', frontUrl);
      const base64Back  = await this.designCanvas.captureViewSnapshot('back',  backUrl);
      ...
      const validDesignId = (savedDesignId && uuidRegex.test(savedDesignId)) ? savedDesignId : null;
      ...
      this.productService
        ...
        .subscribe({
          ...
          error: (error) => {
            ...
            const msg = error?.error?.Message || error?.error?.message || error?.message || 'Failed to save design.';
